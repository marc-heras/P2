...
int main(int argc, char *argv[]) {
  ...
  VAD_DATA *vad_data;
  VAD_STATE state, last_state, maybe_state;
  ...
  for (t = last_t = 0; ; t++) { /* For each frame ... */
    ...
    state = vad(vad_data, buffer, frame_duration);
    ...
    if (state != last_state) {
      ...
      if((state == ST_VOICE && maybe_state == ST_MV) || (state == ST_SILENCE && maybe_state == ST_MS)) {
        if (t != last_t)
          fprintf(vadfile, "%.5f\t%.5f\t%s\n", last_t * frame_duration, maybe_t * frame_duration, state2str(last_state));
        last_state = state;
        last_t = t;
        last_t = maybe_t;
      }
